...
export default function CartDropdown(){
    ...
    const { cartItems, productData, removeItemFromList, getTotalCartItems } = useContext(CartContext);
        ...
        <>
            <main>
                <DropdownMenu>
                    ...
                    <DropdownMenuContent >
                        {cartItems.length !== 0 ? <DropdownMenuLabel className="flex justify-between items-center">
                            <p>Your Cart</p>
                            <AlertDialogPopOver />
                            </DropdownMenuLabel>
                                 : null}
                                    ...
                                    <div key={index}>
                                         <DropdownMenuItem key={index}>
                                            ...
                                            <div className="flex gap-2 items-center">
                                                ...
                                                <div>
                                                    <h2 className="font-semibold capitalize">{cartItem?.item.title}</h2>
                                                    <p>{getCurrencySign(cartItem?.item.price * 100)}</p>
                                                </div>
                                            </div>
                                            <Button onClick={()=>handleRemoveFromCart(index)} className="bg-transparent hover:bg-primary hover:text-white text-primary">Remove</Button>
                                        </div>
                                       </DropdownMenuItem>

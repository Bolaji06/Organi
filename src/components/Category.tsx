        ...
        <>
            <section className="mb-20">
                <div className="py-16 flex flex-col justify-center items-center gap-3">
                    <h1 className="font-extrabold text-xl md:text-2xl lg:text-3xl">Products Category</h1>
                    <div className="w-40 h-1 bg-primary"/>
                    <Link href={'/all-category'} className="inline-flex items-center justify-center 
                    gap-2 hover:text-primary transition-colors duration-200 ease-in-out">View All <ChevronsRight /></Link>
                </div>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-2 place-items-center">
                    { categoryBgImages.map((bgImage, index) => {
                        return (
                            <div
                                key={index} style={{backgroundImage: `url(${bgImage.src})`}}

...
export default function CheckoutCard() {
    ...
    return (
        <>
            <section className="bg-white shadow-md border-2 border-gray-300 rounded-sm
            lg:border-dashed border-spacing-4">
                <div className="border-b border-slate-300 py-1 flex justify-between items-center font-bold text-lg">
                    ...
                    <h2 className="px-2">{cartItems.length} {cartItems.length > 1 ? 'Items' : 'Item'}</h2>
                </div>
                <div className="flex justify-between py-2 border-b border-slate-300">
                    <p className="text-sm px-2">Delivery Charges</p>
                    <p className="text-xs max-w-sm lg:max-w-[100px] px-2">Add your Delivery address at checkout to see delivery charges</p>

                </div>

                <div className="flex justify-between py-2 border-b border-slate-300">
                    <p className="text-sm px-2 ">Subtotal</p>
                    <p className="text-sm px-2 font-bold">{getCurrencySign(getTotalCartItems())}</p>
                </div>

                <div className="flex justify-between py-2 border-b border-slate-300 text-base">
                    <p className="px-2 font-bold">Total</p>
                    <p className="px-2 font-bold">{getCurrencySign(getTotalCartItems())}</p>
                </div>
                <div className="py-2 border-b border-slate-300">
                    <p className="text-xs text-red-800 text-right px-2">Excluding delivery charges</p>
                    <div className="px-2 py-3">
                        <Button className="w-full h-10 rounded-sm font-semibold">Continue to checkout</Button>
                    </div>
                </div>
                ...
                <footer>
                    ...
                    <div className="flex gap-1 py-2 px-1 items-center justify-center">
                        ...
                        <p className="text-xs text-slate-400 text-center">Transactions are 100% secure and safe</p>
                    </div>
                </footer>
            </section>
        </>

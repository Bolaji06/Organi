...
export default function RecentlyViewedProduct(){
    ...
    useEffect(() => {
        async function getData(){
            if (recentViewedProducts){
               const data = await Promise.all(recentViewedProducts.map((item: number) => {
    ...
    return (
        <>
         {
            recentData && <main className="px-4">
            <h2 className="py-4 text-lg md:text-xl text-black font-semibold">Recently Viewed Products</h2>

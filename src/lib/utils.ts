...
export function removeLastSfromCategory(category: string): string {
  const withoutLastS = category.replace(/womens|mens/g, match => (match === 'womens' ? 'women' : 'men'));
  return withoutLastS;
}
export const deliveryService : (string)[] = ['DHL', 'GIGS', 'UPS', 'FedEx']
export const location : (string)[] = ['Lagos', 'Abuja', 'Port Harcourt', 'Ijebu Ode'];
export const categoryBg = [smartphone, laptops, fragrance, skincare, groceries, homeDeco, furniture,
tops, womenDress, womenShoes, menShirt, menShoes, menWatch, womenWatches, womenBag, womenJewllery,
sunglasses, automobile, motorcycle, lamp];
